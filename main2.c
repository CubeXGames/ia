...
void updateStar(void) {
	...
	if(currentSprite->velocityY > 36) currentSprite->velocityY = 36;
}
...
void main(void) {
    ...
    while(TRUE) {
		...
		for(global_i = 0; global_i < 2; ++global_i) {
			...
			if(frameCount >= 100) ++numberOutput;
			else break;
		}
		
		one_vram_buffer(numberOutput + 48, NTADR_A(7, 14));
		
		numberOutput = 0;
		for(global_i = 0; global_i < 10; ++global_i) {
			
			if(frameCount >= 10) ++numberOutput;
			else break;
		}
		
		one_vram_buffer(numberOutput + 48, NTADR_A(8, 14));
		
		numberOutput = 0;
		for(global_i = 0; global_i < 10; ++global_i) {
			
			if(frameCount >= 1) ++numberOutput;
			else break;
		}
		
		one_vram_buffer(numberOutput + 48, NTADR_A(9, 14));
